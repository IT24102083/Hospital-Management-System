document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMedicines() {
        ...
        medicineGrid.innerHTML = filteredMedicines.map(medicine => {
            const isLowStock = medicine.stock < 20 && medicine.stock > 0;
            ...
            <div class="h-full flex flex-col hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border-sky-100 rounded-lg bg-white">
                <div class="relative overflow-hidden rounded-t-lg">
                    ...
                    <div class="absolute top-2 right-2 flex gap-1">
                        ${medicine.requiresPrescription ? `<span class="text-xs bg-red-100 text-red-800 px-2 py-1 rounded-full font-semibold">Rx</span>` : ''}
                        <span class="text-xs bg-white/90 border-sky-200 border px-2 py-1 rounded-full">${medicine.category || 'Uncategorized'}</span>
                    </div>
                </div>
                <div class="p-3 flex-1 flex flex-col">
                    <h3 class="text-lg font-semibold text-gray-900 line-clamp-1" title="${medicine.name}">${medicine.name}</h3>
                    ...
                    <div class="pt-3 border-t bg-sky-50/30 -mx-3 -mb-3 mt-3 p-3">
                         <div class="flex gap-2">
                            <button data-id="${medicine.id}" class="view-details-btn flex-1 bg-white border text-sky-700 font-bold py-2 px-4 rounded text-sm hover:bg-gray-50">Details</button>
    ...
    async function loadData() {
        ...
        try {
            ...
            updateCategoryUI();
            renderCart();
            // showNotification('Could not load medicines. Using sample data.', 'info');
        } finally {
    ...
    // Update category UI (counts and dropdown)
    function updateCategoryUI() {
        ...
        if (categoryFilter) {
            ...
            categoryFilter.innerHTML = categories.map(category => {
                ...
                const count = category === 'all' ? medicines.length : categoryCounts[category];
                return `<option value="${category}">${displayName} (${count})</option>`;
            }).join('');
            categoryFilter.value = selectedCategory;
        }
    }
    ...
    function showMedicineDetails(medicineId) {
        ...
        modalContainer.innerHTML = `
            <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 animate-fade-in">
                <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-fade-in-up">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-2 space-y-6">
                            <div>
                                <h3 class="text-lg font-semibold mb-4 border-b pb-2">Medicine Details</h3>
                                <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                    <span class="text-gray-600">Category:</span><span class="font-medium text-gray-800">${medicine.category || 'Uncategorized'}</span>
                                    <span class="text-gray-600">Dosage:</span><span class="font-medium text-gray-800">${medicine.dosage || 'N/A'}</span>
                                    <span class="text-gray-600">Form:</span><span class="font-medium text-gray-800">${medicine.form || 'N/A'}</span>
                                    <span class="text-gray-600">Requires Rx:</span><span class="font-medium text-gray-800">${medicine.requiresPrescription ? 'Yes' : 'No'}</span>
                                    <span class="text-gray-600">Manufacturer:</span><span class="font-medium text-gray-800">${medicine.manufacturer || 'N/A'}</span>
                                    <span class="text-gray-600">Expires:</span><span class="font-medium text-gray-800">${medicine.expiryDate || 'N/A'}</span>
                                </div>
                            </div>
                            <div>
                                <h3 class="text-lg font-semibold mb-2 border-b pb-2">Description</h3>
                                <p class="text-gray-700 text-sm">${medicine.description || 'No description available.'}</p>
                            </div>
                            <div>
                                <h3 class="text-lg font-semibold mb-3 border-b pb-2">Side Effects</h3>
                                <div class="flex flex-wrap gap-2">${sideEffects}</div>
                            </div>
                             <div>
                                <h3 class="text-lg font-semibold mb-3 border-b pb-2">Contraindications</h3>
    ...
    function addToCart(medicineId) {
        ...
        if (existingItem) {
            ...
            cart.push({ medicineId: Number(medicine.id), name: medicine.name, price: medicine.price, image: medicine.image, quantity: 1 });
        }

        medicine.stock--; // Decrement stock in the local state

        renderCart();
        renderMedicines(); // Re-render to show updated stock
    ...
    // Placeholder functions for cart persistence
    ...
    function init() {
        ...
        document.addEventListener('click', function(e) {
            ...
            // Add to cart from main grid
            if(target.closest('.add-to-cart-btn')) {
                const medicineId = target.closest('.add-to-cart-btn').dataset.id;
                if(medicineId) addToCart(medicineId);
                return;
            }

            // View details
            if(target.closest('.view-details-btn')) {
                const medicineId = target.closest('.view-details-btn').dataset.id;
                if(medicineId) showMedicineDetails(medicineId);
                return;
            }

            // Cart quantity buttons
            if(target.closest('.increment-btn')) {
                const medicineId = target.closest('.increment-btn').dataset.id;
                if(medicineId) updateCartItemQuantity(medicineId, 1);
                return;
            }
            if(target.closest('.decrement-btn')) {
                const medicineId = target.closest('.decrement-btn').dataset.id;
                if(medicineId) updateCartItemQuantity(medicineId, -1);
                return;
            }
            if(target.closest('.remove-btn')) {
                const medicineId = target.closest('.remove-btn').dataset.id;
                if(medicineId) removeFromCart(medicineId);
                return;
            }

            // Checkout button
            if(target.closest('#checkout-btn')) {
                proceedToCheckout(target.closest('#checkout-btn'));
                return;
            }
        });
    ...
    function proceedToCheckout(checkoutBtn) {
        ...
        // The fetch call is now simpler, without CSRF headers
