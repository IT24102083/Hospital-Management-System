...
public class PaymentService {
    ...
    public Payment processCardPayment(Payment.CreateRequest request) {
        try {
            ...
            // Explicitly save and then return the persisted entity to ensure it has the generated ID.
            Payment savedPayment = paymentRepository.save(payment);
            System.out.println("--- DEBUG: Saved Payment ID is: " + savedPayment.getId() + " ---");
            return savedPayment;
    ...
    public Payment processBankTransferPayment(Long invoiceId, BigDecimal amount, String referenceNumber,
        ...
        try {
            ...
            // Step 1: Save the payment first to get a generated ID.
            Payment savedPayment = paymentRepository.save(payment);

            // Step 2: Store the bank slip file if provided, using the now-valid ID.
            if (receiptFile != null && !receiptFile.isEmpty()) {
                String fileName = fileStorageService.storeBankSlip(receiptFile, savedPayment.getId());
                savedPayment.setBankSlipPath(fileName);
                savedPayment.setNotes(savedPayment.getNotes() + " | Bank slip uploaded: " + fileName);

                // Step 3: Update the payment with the file path.
    ...
    @Transactional(readOnly = true)
    public List<Payment> getRecentPOSTransactions() {
